    ...
    item.addEventListener("click", (e) => {
      if (gamestate !== "ON") {
        return Swal.fire({
          icon: "error",
          title: "Oops...",
          text: "¡Juego Finalizado!. Por favor reinicie el juego",
        });
      }
...
function returnBingo() {
  window.location.href = window.location.origin + "/bingo/";
}
...
function checkBingo(item) {
  ...
  if (addmearr.includes(dataint)) {
    return Swal.fire({
      icon: "error",
      title: "Oops...",
      text: "¡Ya has seleccionado este número!",
      toast: true,
      position: "top-right",
    });
  }
...
function loopItemsAndCheck() {
  for (const j of bingoItems) {
    if (includesAll(addmearr, j)) {
      ...
      bingoIndex += 1;
      if (bingoIndex === 5) {
        Swal.fire(loc_username, "¡Felicidades usted ganó!", "success");
        ws.send(
          JSON.stringify({
            command: "won",
            user: loc_username,
            bingoCount: bingoIndex,
            info: `${loc_username} ganó el juego`,
          })
        );
      }
    }
