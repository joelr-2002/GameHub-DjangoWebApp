...
const urls = 'ws://10.241.24.109:8000/ws/clicked/' + room_name + '/';
let gamestate = "ON";
...
window.onbeforeunload = function (event) {
 if(gamestate !== "ON") return;
  return "¿Realmente desea recargar la página?";
};
...
ws.onopen = function (e) {
  ws.send(
    JSON.stringify({
      command: "joined",
      info: `${loc_username} acaba de unirse 👋`,
      user: loc_username,
    })
  );
};
...
ws.onmessage = function (e) {
  ...
  if (command === "joined") {
    ...
    userTurn.textContent =
      currPlayer === loc_username ? "Su turno" : `Turno de: ${currPlayer}`;
  ...
  if (command === "won") {
    gamestate = "OFF";
    if (notForMe(data)) {
      Swal.fire("Usted Pierde", data.info, "error");
    }
  }
  ...
  if (e.key === 13 || e.key === "Enter") {
    if (!chatInput.value.trim()) {
      return Swal.fire({
        icon: "error",
        title: "Oops...",
        text: "¡Oops! No puedes enviar mensajes vacíos",
        toast: true,
        position: "top-right",
      });
    }
