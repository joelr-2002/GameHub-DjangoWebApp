...
create_room.addEventListener('click',async function(){
    try {
        ...
        const check_url = 'http://10.241.24.109:8000/'
        const res = await fetch(`${check_url}bingo/room/check_room/${room_name.value}/`,{
            method:'GET',
        })
        const r = await res.json()
        if(r.room_exist){
            Swal.fire("Sala ocupada", "¡Por favor escoja otra sala!", "error");
        }
        else{
            getInRoom()  
        }
...
const home_urls ="ws://10.241.24.109:8000/ws/online-rooms/" ;
...
socket.onmessage = function (e) {
    ...
    if(data.command==="online_rooms"){
       ...
       onlinerooms.innerHTML ='<p id="no_room" class="room-div">Todavía no hay salas disponibles... 🌵</p>'
